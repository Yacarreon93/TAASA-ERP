...
{   if($fechaTemp != $row->datef) {
    ...
    if($row->fk_cond_reglement != 1) { //facturas a credito
      if($row->total_tva > 0) { //con IVA
        ...
        $VendidoCreditoConIVA[$i] += $row->product_price;
      } else {
        ...
        if($row->total_tva > 0) { //con IVA
          ...
          $VendidoContadoConIVA[$i]+= $row->product_price;
        }else {
...
$pdf->ln();
$pdf->Cell($columnWidth * 4);
$pdf->Cell($columnWidth * 2, $pdf->rowHeight, 'IVA', 1);
$pdf->Cell($columnWidth * 2, $pdf->rowHeight, formatMoney($totalIVA), 1);
$pdf->ln();
$pdf->SetFillColor(...$grayRGB1);
$pdf->Cell($columnWidth * 4);
