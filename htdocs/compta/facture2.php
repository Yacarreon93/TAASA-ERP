			...
			if ($object->fetch($id) > 0) {
				...
				if ($result > 0) {
					if($isTicket) {
						...
						$service->UpdateTicketFields($db, $object->id);
...
else if ($id > 0 || ! empty($ref))
{
	...
	if ($action == 'delete') {
		...
		if ($object->type != Facture::TYPE_DEPOSIT && ! empty($conf->global->STOCK_CALCULATE_ON_BILL) && $qualified_for_stock_change && $object->statut >= 1) {
			...
			if($isTicket) {
				$formconfirm = $form->formconfirm($_SERVER['PHP_SELF'] . '?isTicket=1&facid=' . $object->id, $langs->trans('DeleteBill'), $text, 'confirm_delete', $formquestion, "yes", 1);
			} else {
	...
	if ($action == 'valid')
	{
		...
		if ($object->type != Facture::TYPE_DEPOSIT && ! empty($conf->global->STOCK_CALCULATE_ON_BILL) && $qualified_for_stock_change)
		{
			...
			$service = new ComprobanteCFDIService();
			$isforfacture = $service->IsForFacture($db, $object->id);
			if($isforfacture == 1) {
				$warehouse_array = $warehouse->list_array_ticket();
			}
			else if($user->id == '1' || $user->id == '18') {
	...
	if ($action == 'clone') {
		...
		$formquestion = array(
			...
			array('type' => 'other','name' => 'socid','label' => $langs->trans("SelectThirdParty"),'value' => $form->select_company_tickets()));
// Paiement incomplet. On demande si motif = escompte ou autre
    ...
    window.open(`print_ticket.php?facid='.$object->id.'&copy=1`);">Imprimir Ticket</a></div>';
			...
			// Clone
			if (($object->type == Facture::TYPE_STANDARD || $object->type == Facture::TYPE_DEPOSIT || $object->type == Facture::TYPE_PROFORMA) && $user->rights->facture->creer && !$isTicket)
			{
				print '<div class="inline-block divButAction"><a class="butAction" href="' . $_SERVER['PHP_SELF'] . '?facid=' . $object->id . '&amp;action=clone&amp;object=invoice&amp;isTicket=1">' . $langs->trans("ToClone") . '</a></div>';
			}

			// Crear factura
			if (($object->type == Facture::TYPE_STANDARD || $object->type == Facture::TYPE_DEPOSIT || $object->type == Facture::TYPE_PROFORMA) && $user->rights->facture->creer && $isTicket && $object->paye == 1)
			{
				print '<div class="inline-block divButAction"><a class="butAction" href="' . $_SERVER['PHP_SELF'] . '?facid=' . $object->id . '&amp;action=clone&amp;object=invoice&amp;isTicket=1">Crear Factura</a></div>';
			}
			...
			if ($user->rights->facture->supprimer)
			{
				if (! $object->is_erasable()) {
					...
					if($isTicket) {
						print '<div class="inline-block divButAction"><a class="butActionDelete" href="' . $_SERVER["PHP_SELF"] . '?isTicket=1&facid=' . $object->id . '&amp;action=delete">' . $langs->trans('Delete') . '</a></div>';
					} else {
	...
	else
	{
		...
		if($isTicket) {
			$formmail->param['returnurl'] = $_SERVER["PHP_SELF"] . '?isTicket=true&id=' . $object->id;
		} else {
