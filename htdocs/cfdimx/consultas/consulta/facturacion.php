...
llxHeader('','','','','','','Facturacion Ingresos','',0,0);
...
<div align="center"><strong>
	<h3>Facturaci&oacute;n Ingresos</h3></strong>
</div>
</p>
<form method="post">
Fecha Inicio
...
Fiscal:
<select name="fiscales" >
	<option value="3" <?=getSelected(3, $fiscales)?>>--Todas--</option>
    <option value="1" <?=getSelected(1, $fiscales)?>>Si</option>
	<option value="2" <?=getSelected(2, $fiscales)?>>No</option>
</select>
...
WHERE f.fk_statut = 2 AND f.entity=".$conf->entity." AND f.fk_soc = s.rowid AND (f.datef BETWEEN '".$fecha_actuali."' AND '".$fecha_actualf."' )".$extra_sql."
