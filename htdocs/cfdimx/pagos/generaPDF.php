	...
	function Header(){
		if ($this->print_header){
			...
			*/
			...
			$this->SetFont('Arial','B',7);
			$this->Cell(2,4,strtoupper(utf8_decode("Regimen Fiscal: "). $pdf2->limpiar($this->prm["emisor"]["emisorRegimen"])),0,0,'L');
			$this->Cell(45,4,"",0,0,'C'); //inicio
			$this->Cell(80,4,"",0,0,'L'); //cliente - 1
			$this->Cell(2,4,"",0,0,'C');//enmedio
...
$pdf->Cell(190,4,"Forma de Pago: ".$pagos["formaDePago"],"LR",0,'L'); //cliente
$pdf->Ln();
...
if($pagos["cadPago"]!=NULL && $pagos["cadPago"]){
	...
	$pdf->Cell(190,4,"Cadena del Pago: ".$pagos["cadPago"] ,"LR",0,'L'); //cliente
	$pdf->Ln();
}
if($pagos["selloPago"]!=NULL && $pagos["selloPago"]){
	//$pagos["selloPago"]=$respag->selloPago;
	$pdf->Cell(190,4,"Sello del Pago: ".$pagos["tipoCambioP"] ,"LR",0,'L'); //cliente
	$pdf->Ln();
}
$pdf->Cell(190,4,"","LBR",0,'L'); //cliente
$pdf->Ln(5);
